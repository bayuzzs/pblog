...
@section('main-content')
  <div
      ...
      <div class="hs-dropdown relative inline-flex">
        ...
        <div
          ...
          <button onclick="submitDeleteForm()"
            class="{{ !$Negaras->items() ? 'hidden' : '' }} flex w-full items-center gap-x-3.5 rounded-lg px-3 py-3 text-sm text-gray-800 hover:bg-gray-200 focus:bg-gray-100 focus:outline-none dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:bg-gray-700">
            Hapus Data
          </button>
        </div>
      </div>
    ...
    {{-- Search bar start --}}
    <form action="{{ route('data-master.negara') }}" method="GET"
      ...
      <div class="relative w-full">
        ...
        <input type="text" id="table-search-users" name="search"
          ...
          placeholder="Cari kode Negara" value="{{ request('search') }}">
      </div>
    ...
    {{-- Table start --}}
    <form action="{{ route('data-master.negara') }}" method="POST" class="flex flex-col" id="delete-form" method="POST">
      @csrf
      @method('DELETE')
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse ($Negaras as $negara)
                  <tr data-json="{{ json_encode($negara) }}" onclick="showEditModal(event)"
                    class="cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700">
                    <td class="py-3 ps-4">
                      <div class="flex h-5 items-center">
                        <input id="hs-table-checkbox-1" type="checkbox" name="kodeNegara[]"
                          value="{{ $negara->kodeNegara }}" onclick="event.stopPropagation();"
                          class="rounded border-gray-200 text-blue-600 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:checked:border-blue-500 dark:checked:bg-blue-500 dark:focus:ring-offset-gray-800">
                      </div>
                    </td>
    ...
    @if ($Negaras->hasPages())
      <div class="mt-5 flex items-center justify-between px-3">
        <p class="w-48 break-words text-sm dark:text-gray-400 sm:w-full">
          Data <span class="font-semibold">{{ $Negaras->firstItem() }}</span> ke <span
            class="font-semibold">{{ $Negaras->lastItem() }}</span> dari total <span
            class="font-semibold">{{ $Negaras->total() }}</span> Data Negara
        </p>
        <x-pagination :data="$Negaras" />
      </div>
    @endif
    <!-- End Pagination -->
  </div>
  ...
  <div id="hs-add-modal"
    ...
    <div class="m-3 sm:mx-auto sm:w-full sm:max-w-lg">
      <form action="{{ route('data-master.negara') }}" method="POST">
        @csrf
        <div
          ...
          <div class="custom-scrollbar overflow-y-auto p-4">
            <div class="grid grid-cols-4 items-center gap-y-3">
              <label for="kodeNegara" class="text-sm dark:text-gray-50">Kode Negara</label>
        ...
        <div
          ...
          <div class="custom-scrollbar overflow-y-auto p-4">
            <div class="grid grid-cols-4 items-center gap-y-3">
              ...
              <input type="text" id="kodeNegara" name="kodeNegara"
                class="col-span-3 block w-full rounded-lg border-gray-200 px-4 py-3 text-sm focus:border-blue-500 focus:ring-blue-500 disabled:pointer-events-none disabled:opacity-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:placeholder-gray-500 dark:focus:ring-gray-600"
                placeholder="AA" required readonly>
  ...
  {{-- Modal End --}}
  {{-- Sort Form --}}
  <form id="sort-form" action="{{ route('data-master.negara') }}" class="hidden">
...
@push('script-bawah')
  <script>
    function checkAll(e) {
      document.getElementsByName("kodeNegara[]").forEach((x) => (x.checked = e.checked));
    }
