...
@section('main-content')
  <div
      ...
      <div class="hs-dropdown relative inline-flex">
        ...
        <div
          ...
          <button onclick="submitDeleteForm()"
            class="{{ !$jenis_kemasans->items() ? 'hidden' : '' }} flex w-full items-center gap-x-3.5 rounded-lg px-3 py-3 text-sm text-gray-800 hover:bg-gray-200 focus:bg-gray-100 focus:outline-none dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:bg-gray-700">
            Hapus Data
          </button>
        </div>
      </div>
    ...
    {{-- Search bar start --}}
    <form action="{{ route('data-master.jenis-kemasan') }}" method="GET"
      ...
      <div class="flex gap-1">
        ...
        "extraMarkup": "<div class=\"absolute top-1/2 end-3 -translate-y-1/2\"><svg class=\"flex-shrink-0 size-3.5 text-gray-500 dark:text-gray-500\" xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"m7 15 5 5 5-5\"/><path d=\"m7 9 5-5 5 5\"/></svg></div>"
          ...
          <x-select-option valueInput="kodeKemasan_asc" requestParam="sortOption">Kode Kemasan ↑</x-select-option>
          <x-select-option valueInput="kodeKemasan_desc" requestParam="sortOption">Kode Kemasan ↓</x-select-option>
          <x-select-option valueInput="namaKemasan_asc" requestParam="sortOption">Nama Kemasan ↑</x-select-option>
          <x-select-option valueInput="namaKemasan_desc" requestParam="sortOption">Nama Kemasan ↓</x-select-option>
        </select>
        <!-- End Select -->
        <button type="submit"
    ...
    {{-- Table start --}}
    <form action="{{ route('data-master.jenis-kemasan') }}" method="POST" class="flex flex-col" id="delete-form" method="POST">
      @csrf
      @method('DELETE')
          ...
          <div class="overflow-hidden rounded-lg border shadow dark:border-gray-700 dark:shadow-gray-900">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse ($jenis_kemasans as $jenis_kemasan)
                  <tr data-json="{{ json_encode($jenis_kemasan) }}" onclick="showEditModal(event)"
                    class="cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700">
                    <td class="py-3 ps-4">
                      <div class="flex h-5 items-center">
                        <input id="hs-table-checkbox-1" type="checkbox" name="kodeKemasan[]"
                          value="{{ $jenis_kemasan->kodeKemasan }}" onclick="event.stopPropagation();"
                          class="rounded border-gray-200 text-blue-600 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:checked:border-blue-500 dark:checked:bg-blue-500 dark:focus:ring-offset-gray-800">
                      </div>
                    </td>
                    <td class="flex items-center px-6 py-4 text-gray-800 dark:text-gray-200">
                      {{ $jenis_kemasan->kodeKemasan }}
                    </td>
                    <td class="px-6 py-4 text-sm text-gray-800 dark:text-gray-200">
                      {{ $jenis_kemasan->namaKemasan }}
                    </td>
    ...
    <!-- Pagination -->
    @if ($jenis_kemasans->hasPages())
      <div class="mt-5 flex items-center justify-between px-3">
        <p class="w-48 break-words text-sm dark:text-gray-400 sm:w-full">
          Data <span class="font-semibold">{{ $jenis_kemasans->firstItem() }}</span> ke <span
            class="font-semibold">{{ $jenis_kemasans->lastItem() }}</span> dari total <span
            class="font-semibold">{{ $jenis_kemasans->total() }}</span> Data Kemasan
        </p>
        <x-pagination :data="$jenis_kemasans" />
      </div>
    @endif
    <!-- End Pagination -->
  </div>
  ...
  <div id="hs-add-modal"
    ...
    <div class="m-3 sm:mx-auto sm:w-full sm:max-w-lg">
      <form action="{{ route('data-master.jenis-kemasan') }}" method="POST">
        @csrf
        <div
          ...
          <div class="custom-scrollbar overflow-y-auto p-4">
            <div class="grid grid-cols-4 items-center gap-y-3">
              ...
              <input type="text" id="namaKemasan" name="namaKemasan"
                class="col-span-3 block w-full rounded-lg border-gray-200 px-4 py-3 text-sm focus:border-blue-500 focus:ring-blue-500 disabled:pointer-events-none disabled:opacity-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:placeholder-gray-500 dark:focus:ring-gray-600"
                placeholder="Nama Kemasan" required>
            </div>
          </div>
  ...
  {{-- Modal End --}}
  {{-- Sort Form --}}
  <form id="sort-form" action="{{ route('data-master.jenis-kemasan') }}" class="hidden">
