...
@section('main-content')
		<div
						...
						<div class="hs-dropdown relative inline-flex">
								...
								<div
										...
										<button onclick="submitDeleteForm()"
												class="{{ !$valutas->items() ? 'hidden' : '' }} flex w-full items-center gap-x-3.5 rounded-lg px-3 py-3 text-sm text-gray-800 hover:bg-gray-200 focus:bg-gray-100 focus:outline-none dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:bg-gray-700">
												Hapus Data
										</button>
								</div>
						</div>
				...
				{{-- Search bar start --}}
				<form action="{{ route('data-master.valuta') }}" method="GET"
						...
						<div class="relative w-full">
								...
								<input type="text" id="table-search-users" name="search"
										...
										placeholder="Cari Data Valuta" value="{{ request('search') }}">
						</div>
        ...
        "extraMarkup": "<div class=\"absolute top-1/2 end-3 -translate-y-1/2\"><svg class=\"flex-shrink-0 size-3.5 text-gray-500 dark:text-gray-500\" xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"m7 15 5 5 5-5\"/><path d=\"m7 9 5-5 5 5\"/></svg></div>"
				...
				{{-- Table start --}}
				<form action="{{ route('data-master.valuta') }}" method="POST" class="flex flex-col" id="delete-form" method="POST">
						@csrf
						@method('DELETE')
												...
												<table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
														<thead class="bg-gray-50 dark:bg-gray-700">
																<tr>
																		...
																		<th scope="col"
																				class="px-6 py-3 text-start text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Kurs
																		</th>

																</tr>
														</thead>
														<tbody class="divide-y divide-gray-200 dark:divide-gray-700">
																@forelse ($valutas as $valuta)
																		<tr data-json="{{ json_encode($valuta) }}" onclick="showEditModal(event)"
																				class="cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700">
																				<td class="py-3 ps-4">
																						<div class="flex h-5 items-center">
																								<input id="hs-table-checkbox-1" type="checkbox" name="kodeValuta[]"
																										value="{{ $valuta->kodeValuta }}" onclick="event.stopPropagation();"
																										class="rounded border-gray-200 text-blue-600 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:checked:border-blue-500 dark:checked:bg-blue-500 dark:focus:ring-offset-gray-800">
																						</div>
																				</td>
				...
				@if ($valutas->hasPages())
						<div class="mt-5 flex items-center justify-between px-3">
								<p class="w-48 break-words text-sm dark:text-gray-400 sm:w-full">
										Data <span class="font-semibold">{{ $valutas->firstItem() }}</span> ke <span
												class="font-semibold">{{ $valutas->lastItem() }}</span> dari total <span
												class="font-semibold">{{ $valutas->total() }}</span> Data Valuta
								</p>
								<x-pagination :data="$valutas" />
						</div>
				@endif
				<!-- End Pagination -->
		</div>
		...
		<div id="hs-add-modal"
				...
				<div class="m-3 sm:mx-auto sm:w-full sm:max-w-lg">
						<form action="{{ route('data-master.valuta') }}" method="POST">
								@csrf
								<div
										...
										<div class="custom-scrollbar overflow-y-auto p-4">
												<div class="grid grid-cols-4 items-center gap-y-3">
														<label for="kodeValuta" class="text-sm dark:text-gray-50">Kode Valuta</label>
														<input type="text" id="kodeValuta" name="kodeValuta"
																...
																placeholder="AAA" required>
														<label for="namaValuta" class="text-sm dark:text-gray-50">Nama Valuta</label>
														<input type="text" id="namaValuta" name="namaValuta"
																class="col-span-3 block w-full rounded-lg border-gray-200 px-4 py-3 text-sm focus:border-blue-500 focus:ring-blue-500 disabled:pointer-events-none disabled:opacity-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:placeholder-gray-500 dark:focus:ring-gray-600"
																placeholder="BBB" required>
														<label for="kurs" class="text-sm dark:text-gray-50">Kurs</label>
														<input type="number" id="kurs" name="kurs"
																class="col-span-3 block w-full rounded-lg border-gray-200 px-4 py-3 text-sm focus:border-blue-500 focus:ring-blue-500 disabled:pointer-events-none disabled:opacity-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:placeholder-gray-500 dark:focus:ring-gray-600"
																placeholder="Kurs" required>
												</div>
										</div>
								...
								<div
										...
										<div class="custom-scrollbar overflow-y-auto p-4">
												<div class="grid grid-cols-4 items-center gap-y-3">
														...
														<input type="text" id="namaValuta" name="namaValuta"
																class="col-span-3 block w-full rounded-lg border-gray-200 px-4 py-3 text-sm focus:border-blue-500 focus:ring-blue-500 disabled:pointer-events-none disabled:opacity-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:placeholder-gray-500 dark:focus:ring-gray-600"
																placeholder="Nama Valuta" required>
														<label for="kurs" class="text-sm dark:text-gray-50">Kurs</label>
														<input type="number" id="kurs" name="kurs"
																class="col-span-3 block w-full rounded-lg border-gray-200 px-4 py-3 text-sm focus:border-blue-500 focus:ring-blue-500 disabled:pointer-events-none disabled:opacity-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:placeholder-gray-500 dark:focus:ring-gray-600"
																placeholder="Kurs" required>
		...
		{{-- Modal End --}}
		{{-- Sort Form --}}
		<form id="sort-form" action="{{ route('data-master.valuta') }}" class="hidden">
...
@push('script-bawah')
		<script>
				function checkAll(e) {
						document.getElementsByName("kodeValuta[]").forEach((x) => (x.checked = e.checked));
				}
