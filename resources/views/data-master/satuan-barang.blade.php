...
@section('main-content')
						...
						<div class="hs-dropdown relative inline-flex">
								...
								<div
										...
										<button onclick="submitDeleteForm()"
												class="{{ !$satuanBarangs->items() ? 'hidden' : '' }} flex w-full items-center gap-x-3.5 rounded-lg px-3 py-3 text-sm text-gray-800 hover:bg-gray-200 focus:bg-gray-100 focus:outline-none dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:bg-gray-700">
												Hapus Data
										</button>
								</div>
						</div>
				...
				<form action="{{ route('data-master.satuan-barang') }}" method="GET"
						...
						<div class="relative w-full">
								...
								<input type="text" id="table-search-users" name="search"
										...
										placeholder="Cari Data Satuan Barang" value="{{ request('search') }}">
						</div>
        ...
        "extraMarkup": "<div class=\"absolute top-1/2 end-3 -translate-y-1/2\"><svg class=\"flex-shrink-0 size-3.5 text-gray-500 dark:text-gray-500\" xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"m7 15 5 5 5-5\"/><path d=\"m7 9 5-5 5 5\"/></svg></div>"
										...
										<x-filter-option valueInput="kodeSatuanBarang_asc" requestParam="sortOption">Kode Satuan Barang
												↑</x-filter-option>
										<x-filter-option valueInput="kodeSatuanBarang_desc" requestParam="sortOption">Kode Satuan Barang
												↓</x-filter-option>
										<x-filter-option valueInput="namaSatuanBarang_asc" requestParam="sortOption">Nama Satuan Barang
												↑</x-filter-option>
										<x-filter-option valueInput="namaSatuanBarang_desc" requestParam="sortOption">Nama Satuan Barang
												↓</x-filter-option>
								</select>
								<!-- End Select -->
								<button type="submit"
				...
				{{-- Table start --}}
				<form action="{{ route('data-master.satuan-barang') }}" method="POST" class="flex flex-col" id="delete-form"
						method="POST">
						@csrf
						@method('DELETE')
										...
										<div class="overflow-hidden rounded-lg border shadow dark:border-gray-700 dark:shadow-gray-900">
												<table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
														...
														<tbody class="divide-y divide-gray-200 dark:divide-gray-700">
																@forelse ($satuanBarangs as $satuanBarang)
																		<tr data-json="{{ json_encode($satuanBarang) }}" onclick="showEditModal(event)"
																				class="cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700">
																				<td class="py-3 ps-4">
																						<div class="flex h-5 items-center">
																								<input id="hs-table-checkbox-1" type="checkbox" name="kodeSatuanBarang[]"
																										value="{{ $satuanBarang->kodeSatuanBarang }}" onclick="event.stopPropagation();"
																										class="rounded border-gray-200 text-blue-600 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:checked:border-blue-500 dark:checked:bg-blue-500 dark:focus:ring-offset-gray-800">
																						</div>
																				</td>
																				<td class="flex items-center px-6 py-4 text-gray-800 dark:text-gray-200">
																						{{ $satuanBarang->kodeSatuanBarang }}
																				</td>
																				<td class="px-6 py-4 text-sm text-gray-800 dark:text-gray-200">
																						{{ $satuanBarang->namaSatuanBarang }}
																				</td>
				...
				<!-- Pagination -->
				@if ($satuanBarangs->hasPages())
						<div class="mt-5 flex items-center justify-between px-3">
								<p class="w-48 break-words text-sm dark:text-gray-400 sm:w-full">
										Data <span class="font-semibold">{{ $satuanBarangs->firstItem() }}</span> ke <span
												class="font-semibold">{{ $satuanBarangs->lastItem() }}</span> dari total <span
												class="font-semibold">{{ $satuanBarangs->total() }}</span> Data Satuan Barang
								</p>
								<x-pagination :data="$satuanBarangs" />
						</div>
				@endif
				<!-- End Pagination -->
		</div>
