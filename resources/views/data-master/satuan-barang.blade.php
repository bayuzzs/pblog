...
@section('main-content')
  <div
      ...
      <div class="hs-dropdown relative inline-flex">
        ...
        <div
          ...
          <button onclick="submitDeleteForm()"
            class="{{ !$satuan_barangs->items() ? 'hidden' : '' }} flex w-full items-center gap-x-3.5 rounded-lg px-3 py-3 text-sm text-gray-800 hover:bg-gray-200 focus:bg-gray-100 focus:outline-none dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:bg-gray-700">
            Hapus Data
          </button>
        </div>
      </div>
    ...
    {{-- Search bar start --}}
    <form action="{{ route('data-master.satuan-barang') }}" method="GET"
      ...
      <div class="relative w-full">
        ...
        <input type="text" id="table-search-users" name="search"
          ...
          placeholder="Cari kode Satuan Barang" value="{{ request('search') }}">
      </div>
      <div class="flex gap-1">
        ...
        "extraMarkup": "<div class=\"absolute top-1/2 end-3 -translate-y-1/2\"><svg class=\"flex-shrink-0 size-3.5 text-gray-500 dark:text-gray-500\" xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"m7 15 5 5 5-5\"/><path d=\"m7 9 5-5 5 5\"/></svg></div>"
          ...
          <x-select-option valueInput="kodeSatuanBarang_asc" requestParam="sortOption">Kode Satuan Barang ↑</x-select-option>
          <x-select-option valueInput="kodeSatuanBarang_desc" requestParam="sortOption">Kode Satuan Barang ↓</x-select-option>
          <x-select-option valueInput="namaSatuanBarang_asc" requestParam="sortOption">Nama Satuan Barang ↑</x-select-option>
          <x-select-option valueInput="namaSatuanBarang_desc" requestParam="sortOption">Nama Satuan Barang ↓</x-select-option>
        </select>
        <!-- End Select -->
        <button type="submit"
    ...
    {{-- Table start --}}
    <form action="{{ route('data-master.satuan-barang') }}" method="POST" class="flex flex-col" id="delete-form" method="POST">
      @csrf
      @method('DELETE')
          ...
          <div class="overflow-hidden rounded-lg border shadow dark:border-gray-700 dark:shadow-gray-900">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse ($satuan_barangs as $satuan_barang)
                  <tr data-json="{{ json_encode($satuan_barang) }}" onclick="showEditModal(event)"
                    class="cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700">
                    <td class="py-3 ps-4">
                      <div class="flex h-5 items-center">
                        <input id="hs-table-checkbox-1" type="checkbox" name="kodeSatuanBarang[]"
                          value="{{ $satuan_barang->kodeSatuanBarang }}" onclick="event.stopPropagation();"
                          class="rounded border-gray-200 text-blue-600 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:checked:border-blue-500 dark:checked:bg-blue-500 dark:focus:ring-offset-gray-800">
                      </div>
                    </td>
                    <td class="flex items-center px-6 py-4 text-gray-800 dark:text-gray-200">
                      {{ $satuan_barang->kodeSatuanBarang }}
                    </td>
    ...
    <!-- Pagination -->
    @if ($satuan_barangs->hasPages())
      <div class="mt-5 flex items-center justify-between px-3">
        <p class="w-48 break-words text-sm dark:text-gray-400 sm:w-full">
          Data <span class="font-semibold">{{ $satuan_barangs->firstItem() }}</span> ke <span
            class="font-semibold">{{ $satuan_barangs->lastItem() }}</span> dari total <span
            class="font-semibold">{{ $satuan_barangs->total() }}</span> Data Satuan Barang
        </p>
        <x-pagination :data="$satuan_barangs" />
      </div>
    @endif
    <!-- End Pagination -->
  </div>
  ...
  <div id="hs-add-modal"
    ...
    <div class="m-3 sm:mx-auto sm:w-full sm:max-w-lg">
      <form action="{{ route('data-master.satuan-barang') }}" method="POST">
        @csrf
        <div
          ...
          <div class="custom-scrollbar overflow-y-auto p-4">
            <div class="grid grid-cols-4 items-center gap-y-3">
              <label for="kodeSatuanBarang" class="text-sm dark:text-gray-50">Kode Satuan Barang</label>
              <input type="number" id="kodeSatuanBarang" name="kodeSatuanBarang"
              ...
              <label for="namaSatuanBarang" class="text-sm dark:text-gray-50">Nama Satuan Barang</label>
              <input type="text" id="namaSatuanBarang" name="namaSatuanBarang"
                class="col-span-3 block w-full rounded-lg border-gray-200 px-4 py-3 text-sm focus:border-blue-500 focus:ring-blue-500 disabled:pointer-events-none disabled:opacity-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:placeholder-gray-500 dark:focus:ring-gray-600"
                placeholder="Nama Satuan Barang" required>
            </div>
          </div>
  ...
  {{-- Modal End --}}
  {{-- Sort Form --}}
  <form id="sort-form" action="{{ route('data-master.satuan-barang') }}" class="hidden">
...
@push('script-bawah')
  <script>
    function checkAll(e) {
      document.getElementsByName("kodeSatuanBarang[]").forEach((x) => (x.checked = e.checked));
    }
